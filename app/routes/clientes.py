...
def index():
    ...
    # Convertir a lista de diccionarios para mantener compatibilidad con la plantilla
    clientes_list = []
    for c in clientes:
...
def nuevo():
    print("🔍 DEBUG: Entrando a nuevo, método:", request.method)
    if request.method == "GET":
        print("🔍 DEBUG: Renderizando formulario nuevo")
...
def editar(cliente_id: int):
    print(f"EDITAR: método {request.method}, id {cliente_id}")
    ...
    if request.method == "GET":
        print("Mostrando formulario con datos:", cliente.nombre, cliente.cedula, cliente.email, cliente.telefono)
    ...
    email = (request.form.get("email") or "").strip() or None
    telefono = (request.form.get("telefono") or "").strip() or None
    print("Datos recibidos:", nombre, cedula, email, telefono)
